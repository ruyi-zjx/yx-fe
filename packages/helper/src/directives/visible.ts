...
const VVisible: IVVisibleYx = {
  ...
  install: function (Vue, options) {
    if (!options?.store) return console.warn(DIRE + '注册指令必须传入`vuex`实例，注册失败');
    const dynamicSetting = options.store[options.dynamicStoreKey || 'dynamicSetting'];
    if (!dynamicSetting) return console.warn(DIRE + '未发现存储动态配置的`module`，注册失败');
  ...
  inserted: function (el, binding, vnode) {
    ...
    if (component === 'default') {
      ...
      console.warn(
        `${DIRE} 未发现\`${component}\`组件类型的处理函数，请检查\`${binding.rawName}\`指令语法是否正确`,
      );
    }
  },
};
